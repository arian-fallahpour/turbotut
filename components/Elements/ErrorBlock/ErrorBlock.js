...
const ErrorBlock = ({ className, message, layout = "row", type = "error", hideGradient, ...otherProps }) => {
  return (
    <div className={join(className, classes.ErrorBlock)} {...otherProps}>
      <div className={join(classes.ErrorBlockContainer, classes[layout], classes[type])}>
        ...
        <p className={join("paragraph", "text-center", classes.ErrorBlockMessage)}>{message}</p>
      </div>
    </div>
  );
};
